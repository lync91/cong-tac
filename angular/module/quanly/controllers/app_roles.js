...
app.controller('app_roles', ['$rootScope', '$scope', 'socket', '$http', '$window', '$stateParams', '$modal', function ($rootScope, $scope, socket, $http, $window, $stateParams, $modal) {
    $scope.roles = [];
    socket.emit('get app roles', function (data) {
        console.log(data);
        $scope.roles = data
    });
    ...
    $scope.add = function () {
        // console.log('OK');
        var modalInstance = $modal.open({
            templateUrl: 'module/quanly/modals/add_app_roles.htm',
            controller: 'add',
            resolve: {}
        });
...
app.controller('add', ['$scope', 'socket', '$modalInstance', function ($scope, socket, $modalInstance) {
    $scope.ok = function () {
        console.log($scope.r);
        socket.emit('add app role', $scope.r, function (res) {
