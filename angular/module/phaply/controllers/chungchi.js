...
/* Controllers */
// projects controller
app.controller('chungchi', ['$scope', 'socket', '$http', 'applib', '$resource', "$filter", '$modal', 'FileUploader', function($scope, socket, $http, applib, $resource, $filter, $modal, FileUploader) {
    $scope.Math = Math;
    ...
    $scope.edit_nhansu = function(cv) {
        // console.log(u);
        var modalInstance = $modal.open({
            templateUrl: 'module/phaply/modals/edit_nhansu.htm',
            controller: 'edit_nhansu',
            size: '',
            resolve: {
              cv: function () {
                return cv;
...
app.controller('edit_chungchi', ['$scope', '$modalInstance', '$log', 'socket', 'u', 'lct', 'lcc', function($scope, $modalInstance, $log, socket, u, lct, lcc) {
    // console.log(lct);
    $scope.loaicongtrinh = lct;
    $scope.loaichungchi = lcc;
    $scope.hang = [{hang: 1}, {hang: 2}, {hang: 3}];
...
app.controller('edit_nhansu', ['$scope', '$modalInstance', '$log', 'socket', 'cv','trinhdo', function($scope, $modalInstance, $log, socket, cv, trinhdo) {
    // console.log(lct);
    console.log(trinhdo);
    $scope.u = cv;
    $scope.u.fullname_new = cv.fullname;
    $scope.u.trinhdo_new = cv.trinhdo;
    $scope.trinhdo = trinhdo;
    // $scope.hang = [{hang: 1}, {hang: 2}, {hang: 3}];
    $scope.open = function($event) {
        $event.preventDefault();
    ...
    $scope.ok = function() {
        socket.emit('edit_canhan', $scope.u, function(data) {
            cv.fullname = $scope.u.fullname_new;
            cv.trinhdo = $scope.u.trinhdo_new;
            $modalInstance.close(data);
        });
    };
    $scope.cancel = function() {
