...
app.controller('milestones', ['$rootScope', '$scope', 'socket', '$http', '$window', '$stateParams', '$modal', 'editableOptions', 'editableThemes', '$state',
  function ($rootScope, $scope, socket, $http, $window, $stateParams, $modal, editableOptions, editableThemes, $state) {
    ...
    $scope.get_ms($stateParams.p_id);
    $scope.selected_m = {};
    $scope.chitiet = function (m) {
      // console.log(m);
      $scope.selected_m = m;
      $state.go('app.projects.view.milestones.details', {
        mid: m.milestone_num
      });
    };

    $rootScope.$on('update_m', function (e, p_id, mid) {
      console.log(mid);
      // $scope.get_ms(mid);
      // socket.emit('get_updated_m', mid, function (data) {
      //   console.log($scope.selected_m);
      //   if (data) {
      //     console.log("OK");
      //     $scope.selected_m.sovanban = data.sovanban;
      //     $scope.selected_m.ngayph = data.ngayph;
      //     $scope.selected_m.trangthai = data.trangthai;
      //     $scope.selected_m.slluu = data.slluu;
      //   }
      // });
      socket.emit('get_project_milestones', $stateParams.p_id, function (data) {
        $scope.milestones = data;
        console.log(data);
      });
    });
    $rootScope.$on('deleted_m', function () {
      $scope.selected_m.deleted = true;
    });
    ...
    //     //     $scope.project = data;
    //     // });
    //     $state.go('app.projects.view.milestones', {p_id : $stateParams.p_id, mid: null })
    //   });
    // };
    ...
    $scope.sortable_option = {
      ...
      stop: function (list, dropped_index) {
        ...
        // console.log(list[ dropped_index])
        socket.emit('update_milestones_sort', list, function (data) {
    ...
    // Lưu file scan
    $scope.add_scanfile = function (l, it, i, m) {
      console.log(it)
      socket.emit('save_milestone_scan', {
        f: it,
        m: m._id
      }, function (err) {
        if (!err) {
...
app.controller('tao_giaonhan', function ($modalInstance, $scope, $rootScope, socket, listm, p, diadiem, $stateParams, $window, $timeout) {
  ...
  $scope.alert = {
    type: 'success',
    msg: 'Khởi tạo biên bản bàn giao hồ sơ thành công !'
  };
  ...
  $scope.print = function () {
    ...
    socket.emit('get logo', 1, function (data) {
      $scope.logo = data;
      socket.emit('get_qr', "http://namviet.tk/api/m/", function (data) {
        // console.log(data);
...
app.controller('milestone', function ($modalInstance, $modal, $scope, $rootScope, $stateParams, FileUploader, socket, $timeout) {
  ...
  $scope.dsphapnhan = [];
  $scope.dschudautu = [];
  $scope.get_hosoluu = function (mid) {
    console.log(mid);
    socket.emit('get_phong_luu_hs', mid, function (data) {
      console.log(data);
      $scope.hosoluu = data;
    });
  };
  $scope.lichsu_trangthai = {}
  $timeout(function () {
    socket.emit('get_milestone', $stateParams.mid, function (data) {
      console.log(data);
      $scope.m = data;
      $scope.get_hosoluu(data._id);
      console.log($stateParams.mid)
    });
    socket.emit('get_project_details', $stateParams.p_id, function (data) {
      $scope.p = data;
    ...
    socket.emit('get_lichsu_trangtahi', $stateParams.mid, function (data) {
      console.log(data);
      if (data) {
        data.forEach(function (t, i) {
          $scope.lichsu_trangthai[t.trangthai.class] = t;
        });
        console.log($scope.lichsu_trangthai)
      }
    });
  ...
  $scope.update_m = function (mm) {
    mm.p_id = $stateParams.p_id;
    socket.emit('update_m', mm, function (data) {
      // console.log(data);
      $rootScope.$emit('update_m', '3,' ,$stateParams.mid);
  ...
  $scope.update_hd = function (hd) {
    socket.emit('update_hd', hd, function (data) {
      ...
      $rootScope.$emit('update_m', 'e', $stateParams.mid);
    });
  };
  ...
  //Giao nhận hồ sơ
  $scope.giaonhanhs = function (size, parentSelector, mm, ac, a) {
    var parentElem = parentSelector ?
      angular.element($document[0].querySelector('.modal-demo ' + parentSelector)) : undefined;
    var modalInstance = $modal.open({
      animation: true,
    ...
    modalInstance.result.then(function (data) {
      bbgiaonhan(data.hsl);
      // $scope.m.slluu = $scope.m.slluu;
      // m.trangthai_doc = [$scope.m.trangthai.trangthai];
      $scope.get_hosoluu($scope.m._id);
    }, function () {
      console.log('OK');
    });
  };
...
app.controller('giaoviec', function ($modalInstance, $state, $scope, $rootScope, socket, m, nv, pb, u) {
  // console.log(m)
  $scope.u = u;
  $scope.t = {};
  if (!u) {
...
app.controller('bbgiaonhan', function ($modalInstance, $scope, $rootScope, socket, bb, p, $timeout, $window) {
  $scope.bb = bb;
  $scope.p = p;
  console.log(p)
  socket.emit('get_qr', "http://namviet.tk/api/m/", function (data) {
...
app.controller('chitiet_nhiemvu', function ($modalInstance, $scope, $rootScope, socket, task, m) {
  console.log(m);
  $scope.user_details = $rootScope.globals.user_details;
  $scope.t = task;
...
app.controller('bao_duyet', function ($stateParams, $modalInstance, $scope, $rootScope, socket, p, m, ac) {
  ...
  $scope.ok = function () {
    socket.emit('bao_duyet', m._id, function (d) {
      ...
      $modalInstance.close(d.m.trangthai);
    });
  };
  $scope.cancel = function () {
    $modalInstance.dismiss('cancel');
...
app.controller('giaonhanhs', function ($modalInstance, $scope, $rootScope, socket, p_id, m, nv, pb, ac, a) {
  $scope.nv = nv;
  var hs = {
    milestone_name: m.milestone_name,
