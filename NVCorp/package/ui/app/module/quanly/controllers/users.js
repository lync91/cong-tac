...
app.controller('users', ['$rootScope', '$scope', 'socket', '$http', '$window', '$stateParams', '$modal', function ($rootScope, $scope, socket, $http, $window, $stateParams, $modal) {
    ...
    $scope.link_user_zalo = function (u) {
        var modalInstance = $modal.open({
            templateUrl: 'module/quanly/modals/link_zalo.htm',
            controller: 'link_user_zalo',
            resolve: {
                u: function () {
                    return u;
                }
            ...
            resolve: {
                ...
                ac: function () {
                    return {
                        ...
                        noti: 'Nhân viên này sẽ khoá tìa khoản đăng nhập!',
                        ok_button: 'Khoá tài khoản'
                    }
                }
            }
        });
            ...
            resolve: {
                ...
                ac: function () {
                    return {
                        ...
                        noti: 'Nhân viên này sẽ được mở khoá đăng nhập!',
                        ok_button: 'Mở khoá tài khoản'
                    }
                }
            }
        });
...
app.controller('edit_user', ['$scope', 'socket', '$modalInstance', 'u', 'chucvu', 'phongban', function ($scope, socket, $modalInstance, u, chucvu, phongban) {
    ...
    $scope.u.phongtructhuoc_new = [];
    // $scope.u.phongtructhuoc_new = u.phongtructhuoc;
    $scope.phongban.forEach(function (pb, i) {
        u.phongtructhuoc.forEach(function (pbb, i) {
            if (pbb._id === pb._id) {
                $scope.u.phongtructhuoc_new.push(pb)
            }
        });
    });
    $scope.delete_zalo = function (u) {
        socket.emit('delete zalo id', u, function (res) {
            u.zalo = "";
        });
    };
    $scope.ok = function () {
...
app.controller('link_user_zalo', ['$scope', 'socket', '$modalInstance', 'u', function ($scope, socket, $modalInstance, u) {
    // console.log(u.company);
    // console.log(u);
    console.log(u);
    socket.emit('get_zalo_token', u, function (res) {
        console.log(res.user);
        var tk = "";
        tk = res._id;
        socket.emit('get_qr', tk, function (qr) {
            console.log(qr);
            $scope.qr = qr;
        });
    });
...
app.controller('user_role', ['$scope', 'socket', '$modalInstance', 'u', function ($scope, socket, $modalInstance, u) {
    ...
    socket.emit('get_roles', 1, function (roles) {
        $scope.u_roles = u.roles ? JSON.parse(u.roles) : {}
        console.log($scope.u_roles);
        $scope.roles = roles;
        console.log(roles);
...
app.controller('disable_user', ['$scope', 'socket', '$modalInstance', 'u', 'ac', function ($scope, socket, $modalInstance, u, ac) {
    $scope.u = u;
    $scope.ac = ac;
    $scope.ok = function () {
        socket.emit('disable_user', u, function (res) {
            u.disable = true;
            $modalInstance.close();
        })
    };
...
app.controller('enable_user', ['$scope', 'socket', '$modalInstance', 'u', 'ac', function ($scope, socket, $modalInstance, u, ac) {
    $scope.u = u;
    $scope.ac = ac;
    $scope.ok = function () {
        socket.emit('enable_user', u, function (res) {
            u.disable = false;
            $modalInstance.close();
        })
    };
