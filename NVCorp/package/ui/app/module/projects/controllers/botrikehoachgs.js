...
app.controller('botrikehoachgiamsat', ['$rootScope', '$scope', 'socket', '$http', '$location', '$modal', '$stateParams', '$window', '$timeout', '$log', 'ganttUtils', 'GanttObjectModel', 'ganttMouseOffset', 'ganttDebounce',
  function ($rootScope, $scope, socket, $http, $location, $modal, $stateParams, $window, $timeout, $log, ganttUtils, GanttObjectModel, ganttMouseOffset, ganttDebounce) {
      ...
      socket.emit('get ke hoach details', $stateParams.kh_id, function (data) {
        ...
        if (angular.isArray(data.dscongtac)) {
          data.dscongtac.forEach(e => {
            ...
            if (e.khbatdau) {
              if (!view) {
                var d = new Date(e.khbatdau);
                $scope.cur_mouth = (Number(d.getMonth()) + 1).toString();
                $scope.cur_year = (Number(d.getFullYear())).toString();
                $scope.change_cur_mouth($scope.cur_mouth, $scope.cur_year);
                view = true;
