...
app.controller('FullcalendarCtrl', ['$scope', '$stateParams', 'socket', '$window', '$timeout', '$log', 'ganttUtils', 'GanttObjectModel', 'ganttMouseOffset', 'ganttDebounce',
  function ($scope, $stateParams, socket, $window, $timeout, $log, ganttUtils, GanttObjectModel, ganttMouseOffset, ganttDebounce) {
    ...
    var today = new Date();
    $scope.cur_mouth = (today.getMonth() + 1).toString();
    console.log($scope.cur_mouth);
    $scope.cur_year = today.getFullYear().toString();
    var lastyear = Number($scope.cur_year) - 1;
    var nextyear = (Number($scope.cur_year) + 1);
    $scope.lsyear = [lastyear, $scope.cur_year, nextyear]
    ...
    $scope.options = {
      ...
      daily: true,
      maxHeight: function () {
        var win = $scope.getWindowDimensions();
        return win.h - 100;
      },
      width: false,
      zoom: 1,
      ...
      fromDate: new Date(2018, 1, 1, 11, 0, 0),
      toDate: new Date(2018, 1, 31, 15, 0, 0),
      ...
      api: function (api) {
        ...
        api.core.on.ready($scope, function () {
          ...
          if (api.tasks.on.drawBegin) {
            api.tasks.on.drawBegin($scope, function (e) {
              addEventName('tasks.on.drawBegin', logTaskEvent);
              // console.log(e.row);

            })
            // api.tasks.on.draw($scope, addEventName('tasks.on.draw', logTaskEvent));
            api.tasks.on.drawEnd($scope, function (e) {
              addEventName('tasks.on.drawEnd', logTaskEvent)
              // console.log(e.row.model);
              // $scope.data.forEach(function(item){
              //   if (item.id == e.row.model.id) {
              //     item.tasks = [e.model];
              //   }
              // });
              // var data = {
              //   _id: e.row.model.id,
              //   khbatdau: e.row.model.tasks[0].from,
              //   khketthuc: e.row.model.tasks[0].to
              // }
              // $scope.update_tiendo(data);
            })
          }
          ...
          api.directives.on.new($scope, function (directiveName, directiveScope, element) {
            ...
            } else if (directiveName === 'gantt-task') {
              element.bind('click', function () {
                logTaskEvent('task-click', directiveScope.task)
              })
            }
    ...
    $scope.update_tiendo = function (data) {
      socket.emit('update ke hoach tien do cong tac giam sat', data, function (res) {
        console.log(res);
      })
    }
    $scope.change_cur_mouth = function (cur_mouth, cur_year) {
      console.log(cur_mouth);
      $scope.options.fromDate = new Date(cur_year, cur_mouth - 1, 1)
      $scope.options.toDate = new Date(cur_year, cur_mouth - 1, 31)
    }
    ...
    $scope.load = function () {
      ...
      // $scope.timespans = getSampleTimespans()
    }
    ...
    function loadData() {
      var dt = [];
      socket.emit('get cong tac thang cac nhan vien', {}, function (data) {
        console.log(data);
        if (angular.isArray(data)) {
          data.forEach(function (e, i) {
            var us = [];
            dt.push({
              name: "Phòng " + e._id.company_name,
              id: e._id._id,
              // tasks: [{
            ...
            if (angular.isArray(e.users)) {
              e.users.forEach(function (u, ii) {
                var cts = [];
                u.dscongtac.forEach(function (ct, cti) {
                  if (ct.dscongtac) {
                    console.log(ct);

                    cts.push({
                      id: ct.dscongtac._id,
                      name: ct.project.project_code,
                      color: '#23b7e5',
                      from: ct.dscongtac.khbatdau,
                      to: ct.dscongtac.khketthuc
                    })
                  }
                })
                dt.push({
                  id: u._id,
                  name: u.fullname,
                  parent: "Phòng " + e._id.company_name,
                  tasks: cts
                })
              })
            }
